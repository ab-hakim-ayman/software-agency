  ...
  const [userToken] = useState(localStorage.getItem('DTCAdminToken'));
    ...
    const handleDelete = (e, id) => {
      if(window.confirm('Are you sure ?')){
          e.preventDefault();
          axios.delete(Base_URL+'/api/admin-it/banner/' + id + '/', {
            method: 'DELETE',
            headers: {
    ...
    const handleUpdate = (e, id) => {
      navigate(`/admin/it/update-banner/${id}/`);
    };

    const fetchData = async () => {
      try {
        await axios.get(Base_URL+'/api/admin-it/banner/', {
          headers: {
            Authorization: 'Bearer ' + userToken
          }
        })
        ...
        <div className="flex justify-between items-center mt-10 ml-1 uppercase text-lg tracking-wide">
          ...
          <Form>
              <Form.Group className="mb-3">
                <Form.Control
                  className="p-2 text-white bg-blue-950 border-[0.3px] border-slate-400 shadow"
                  type="text"
