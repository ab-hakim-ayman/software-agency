...
import { AdminUnAuth } from "../../../../Auth_Middleware/UnAuth";
  ...
  const Technology = () => {
    const [userToken] = useState(localStorage.getItem('DTCAdminToken'));
      ...
      const itemsToShow = technology.slice(startIndex, endIndex);
  
      const handleDelete = async (e, id, activeValue) => {
        if(window.confirm('Are you sure ?')){
            e.preventDefault();
            await axios.patch(Base_URL+'/api/admin-it/technology-active-deactive/' + id + '/',  {active:activeValue?true:false},{
                headers: {
      ...
      const handleUpdate = (e, id) => {
        navigate(`/admin/it/update-technology/${id}/`);
      };
  
      const fetchData = async () => {
        try {
          await axios.get(Base_URL+'/api/admin-it/technology/', {
            headers: {
              Authorization: 'Bearer ' + userToken
            }
          })
      ...
      return (
          ...
          <div className="flex items-center justify-between">
            ...
            <Link to="/admin/it/add-technology">
            ...
            <table className="table  border-[0.3px] border-slate-500  mt-10 h-[full]  bg-blue-900  rounded-sm">
              ...
              <thead className="text-white  bg-blue-950">
                <tr>
                  ...
                  <th>
                    <span className=" border-l-2  border-slate-200  pl-2">Path</span>
                  </th>
                  <th>
