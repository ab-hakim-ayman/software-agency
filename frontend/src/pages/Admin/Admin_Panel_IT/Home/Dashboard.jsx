...
const Dashboard = () => {
  ...
  const [userToken] = useState(localStorage.getItem('DTCAdminToken'));
    ...
    useEffect(() => {
      const fetchData = async () => {
        try {
          await axios.get(Base_URL+'/api/admin-it/get-dashboard/', {
            headers: {
  ...
  return (
    <div className=" bg-blue-950  px-10 w-full overflow-y-scroll pb-10 h-full mt-12 text-white">
      <h1 className="text-5xl text-orange-300 uppercase font-semibold mt-4">
        Dashboard
      </h1>
      ...
      <div className='block w-full  py-8 lg:flex lg:gap-2'>
        <div className='border border-slate-400 rounded-md bg-blue-950 text-slate-200 w-full pb-16 lg:w[50%]'>
