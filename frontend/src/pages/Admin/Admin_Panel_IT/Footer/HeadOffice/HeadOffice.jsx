  ...
  const HeadOffice = () => {
      ...
      const handleDelete = (e, id, activeValue) => {
        if(window.confirm('Are you sure ?')){
            e.preventDefault();
            axios.patch(Base_URL+'/api/admin-it/head-office/' + id + '/',{active:activeValue?true:false},{
                headers: {
      ...
      const handleUpdate = (e, id) => {
        navigate(`/admin/it/update-head-office/${id}/`);
      };
  
      const fetchData = async () => {
        try {
          await axios.get(Base_URL+'/api/admin-it/head-office/',{
            headers: {
                'Accept': 'application/json',
                'Content-Type': 'application/json',
                Authorization: 'Bearer ' + userToken
