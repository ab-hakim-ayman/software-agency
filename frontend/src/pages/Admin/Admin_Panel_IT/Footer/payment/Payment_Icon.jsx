  ...
  const [userToken] = useState(localStorage.getItem('DTCAdminToken'));
    ...
    const handleDelete = async(e, id,activeValue) => {
      if(window.confirm('Are you sure ?')){
          e.preventDefault();
          await axios.patch(Base_URL+'/api/admin-it/payment-icon/' + id + '/', {active:activeValue?true:false},{
            headers: {
    ...
    const handleUpdate = (e, id) => {
      navigate(`/admin/it/update-payment/${id}/`);
    };

    const fetchData = async () => {
      try {
        await axios.get(Base_URL+'/api/admin-it/payment-icon/', {
          headers: {
            Authorization: 'Bearer ' + userToken
          }
        }).then((response)=>{
