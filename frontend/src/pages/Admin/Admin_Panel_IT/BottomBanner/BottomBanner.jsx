...
const BottomBanner = () => {
  const [userToken] = useState(localStorage.getItem('DTCAdminToken'));
    ...
    const [currentPage, setCurrentPage] = useState(1);
      ...
      const totalItems = bottomBanner.length;
      ...
      const itemsToShow = bottomBanner.slice(startIndex, endIndex);

    const handleDelete = (e, id) => {
      if(window.confirm('Are you sure ?')){
          e.preventDefault();
          axios.delete(Base_URL+'/api/admin-it/bottom-banner/' + id + '/', {
            method: 'DELETE',
            headers: {
    ...
    const handleUpdate = (e, id) => {
      navigate(`/admin/it/update-bottom-banner/${id}/`);
    };

    const fetchData = async () => {
      try {
        await axios.get(Base_URL+'/api/admin-it/bottom-banner/', {
          headers: {
            Authorization: 'Bearer ' + userToken
          }
        })
        .then((response) =>{
          ...
          toast.error("Banner fetching failed!");
        })          
      } catch (error) {
    ...
    return (
        ...
        <div className="flex items-center justify-between">
          ...
          <Link to="/admin/it/add-bottom-banner">
