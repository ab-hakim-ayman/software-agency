  ...
  const PaymentMethod = () => {
    const [userToken] = useState(localStorage.getItem('DTCAdminToken'));
      ...
      const [currentPage, setCurrentPage] = useState(1);
        ...
        const itemsToShow = payment.slice(startIndex, endIndex);
  
      const handleDelete = (e, id, activeValue) => {
        if(window.confirm('Are you sure ?')){
            e.preventDefault();
            axios.patch(Base_URL+'/api/admin-it/payment-method/' + id + '/', {active:activeValue?true:false}, {
              headers: {
      ...
      const handleUpdate = (e, id) => {
        navigate(`/admin/it/update-payment-method/${id}/`);
      };
  
      const fetchData = async () => {
        try {
          await axios.get(Base_URL+'/api/admin-it/payment-method/', {
            headers: {
              Authorization: 'Bearer ' + userToken
            }
          })
          .then((response) =>{
            ...
            toast.error("Payment fetching failed!");
          })          
        } catch (error) {
      ...
      return (
          ...
          <div className="flex items-center justify-between">
            ...
            <Link to="/admin/it/add-payment-method">
            ...
            <table className="table  border-[0.3px] border-slate-500  mt-10 h-[full]  bg-blue-900  rounded-sm">
              ...
              {itemsToShow.filter((item) =>{
                ...
                <tr key={t.id}>
                    <td>{t?.id}</td>
                    <td>{t?.method_name}</td>
                    <td className="font-semibold">{t.active ? "Active" : "Disable"}</td>
                    <td>
                        ...
                        <span>&nbsp;&nbsp;&nbsp;</span>
                        <Button className="mr-2" onClick={(e) => handleUpdate(e, t.id,t.active?false:true)}>
                            <FaEdit />
                        </Button>
                    </td>
                </tr>
