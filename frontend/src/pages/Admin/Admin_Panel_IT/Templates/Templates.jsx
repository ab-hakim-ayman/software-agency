...
const Templates = () => {
  const [userToken] = useState(localStorage.getItem('DTCAdminToken'));
    ...
    const [currentPage, setCurrentPage] = useState(1);
      ...
      const itemsToShow = template.slice(startIndex, endIndex);

    const handleDelete = (e, id,activeValue) => {
      if(window.confirm('Are you sure ?')){
          e.preventDefault();
          axios.patch(Base_URL+'/api/admin-it/home-template/' + id + '/',  {active:activeValue?true:false},{
            headers: {
    ...
    const fetchData = async () => {
      try {
        await axios.get(Base_URL+'/api/admin-it/home-template/', {
          headers: {
            Authorization: 'Bearer ' + userToken
          }
        })
        .then((response) =>{
          ...
          toast.error("Templates fetching failed!");
        })          
      } catch (error) {
