  ...
  const [userToken] = useState(localStorage.getItem('DTCAdminToken'));
    ...
    useEffect(() => {
      const fetchData = async () => {
        try {
          await axios.get(Base_URL+`/api/admin-it/readmore/${id}/`, {
            headers: {
    ...
    const handleSubmit = (e) => {
      e.preventDefault();
      try {
        axios.patch(Base_URL+'/api/admin-it/readmore/'+ id + '/', formData, {
          headers: {
        ...
        .then((response) =>{
          if (response.data.type === 'success'){
            ...
            navigate('/admin/it/readmore')
          } else {
            toast.error(response.data.msg)
          }
        }).catch((error) =>{
          toast.error("Readmore update failed!")
        })
      } catch (error) {
        toast.error("Network error occured!")
      }  
