...
const All = () => {
  ...
  const handleUpdate = (e) => {
    const data =  {state: e}
    navigate(`/admin/it/update-order`, data );
  };

  const handleUploadOrderFile = (e) => {
    const data =  {state: e}
    navigate(`/admin/it/upload-order`, data );
  };
  ...
  const workingOrder = (e, id) => {
    if(window.confirm('Do you want to change order status, working ? \nYou can cancel the order later')){
      e.preventDefault();
      axios.patch(Base_URL+'/api/admin-it/working-order/' + id + '/',{}, {
        headers: {
          'Accept': 'application/json',
          'Content-Type': 'application/json',
          Authorization: 'Bearer ' + userToken
        ...
        .then((response)=>{
          if (response.data.type === 'success'){
            ...
            fetchData()
            navigate('/admin/it/working-orders')
          }else{
            toast.error(response.data.msg)
          }
        }).catch((e)=>{
  ...
  const CompleteOrder = (e, id) => {
    ...
    const fetchData = async () => {
      try {
        await axios.get(Base_URL+'/api/admin-it/orders/',{
          headers: {
              'Accept': 'application/json',
              'Content-Type': 'application/json',
              Authorization: 'Bearer ' + userToken
          ...
          <table className="table  border-[0.3px] border-slate-500  mt-10 h-[full]  bg-blue-900  rounded-sm">
            ...
            {itemsToShow.filter((item) =>{
              ...
              <tr key={t.id}  >
                  ...
                  <td>{(t.status === 'pen') ? 'Pending' : (t.status === 'pay') ? 'Payment' : (t.status === 'can') ? 'Cancelled' : (t.status === 'del') ? 'Delivered' : (t.status === 'com') ? 'Completed' : 'Working'}</td>
                  <td>{t.status === 'pen' && <>
                  ...
                  {t.status === 'com' && <>
                      {(t.total_online_paid + t.total_offline_paid) ===  t.total_price  && <button onClick={()=>handleUploadOrderFile(t)}
