  ...
  const Account = () => {
    const [userToken] = useState(localStorage.getItem('DTCAdminToken'));
      ...
      const [currentPage, setCurrentPage] = useState(1);
        ...
        const totalItems = account.length;
        ...
        const itemsToShow = account.slice(startIndex, endIndex);
  
      const handleDelete = (e, id,activeValue) => {
        if(window.confirm('Are you sure ?')){
            e.preventDefault();
            axios.patch(Base_URL+'/api/admin-it/account-active-disable/' + id + '/', {active:activeValue?true:false}, {
              headers: {
      ...
      const handleUpdate = (e, id) => {
        navigate(`/admin/it/update-account/${id}/`);
      };
  
      const fetchData = async () => {
        try {
          await axios.get(Base_URL+'/api/admin-it/account/', {
            headers: {
              Authorization: 'Bearer ' + userToken
            }
          })
          .then((response) =>{
            ...
            toast.error("Account fetching failed!");
          })          
        } catch (error) {
      ...
      return (
          ...
          <div className="flex justify-between items-center mt-10 ml-1 uppercase text-lg tracking-wide">
            ...
            <Form>
                <Form.Group className="mb-3">
                  <Form.Control
                    className="py-2 px-4 text-white bg-blue-950 shadow outline-none border-[0.3px] border-slate-400"
                    type="text"
          ...
          <div className="overflow-x-auto rounded-md">
            <table className="table border-[0.3px] border-slate-500 rounded-md mt-10 h-[full]  bg-blue-900 ">
              {/* head */}
              ...
              {itemsToShow.filter((item) =>{
                  return search.toLowerCase() === '' ? item : item.bank_name.toLowerCase().includes(search);   
                }).map((t,i) =>
                <tr key={t.id} className={`${i%2 === 0? " bg-[#1b2649] " : " bg-blue-950 "} `}>
                    <td>{t.id}</td>
                    <td><img src={t?.bank_img} alt="" height="30px" width="30px"/></td>
                    <td><img src={t?.bar_code} alt="" height="30px" width="30px"/></td>
                    <td>{t.bank_name}</td>
                    <td>{t.country}</td>
                    <td>{t.payment_method.method_name}</td>
