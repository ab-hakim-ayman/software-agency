...
const BottomBanner = () => {
  ...
  const DataFetch=()=>{
    axios.get(Base_URL+"/api/civil-admin/bottom-banner/", config)
  ...
  const deleteButtonClick = (e, id) => {
    e.preventDefault()
    axios.delete(Base_URL+`/api/civil-admin/bottom-banner/${id}/`, config)
      .then(res => {
  ...
  return (
    <div className=" bg-slate-950  px-10 w-full h-full mt-12 text-white">
      <div className="flex items-center justify-between">
        ...
        <Link to="/admin/civil/addBottomBanner">
          ...
          {data?.data?.map((item, i) => {
              return <>
                ...
                <td class="px-6 py-4">
                  ...
                  <Link to={`/admin/civil/editBottomBanner/${item?.id}/`}>
