...
const EditBottomBanner = () => {
  ...
  useEffect(() => {
    axios.get(Base_URL+`/api/civil-admin/bottom-banner/${id}/`, config)
      .then(res => {
  ...
  const UpdateBanner = (e) => {
    ...
    axios.patch(Base_URL+`/api/civil-admin/bottom-banner/${id}/`, data1, config)
      .then(response => {
        console.log(response.data)
        toast.success('Bottom Banner edited Successfully')
        setTimeout(() => {
          navigate('/admin/civil/bottombanner');
        }, 1000);
        })
      .catch(err => console.log(err));
  }
  ...
  return (
    <div className=" bg-slate-950  px-10 w-full h-full mt-12 text-white">
      ...
      <div className="mt-8">
        ...
        <textarea  className="h-32 px-4 py-4 w-full  bg-slate-950  border rounded border-neutral-500 " placeholder="Enter Description"
          value={description}
      ...
      <div className="flex items-center justify-center mb-4">
        <button className="btn rounded h-8 bg-gray-500 shadow-inherit border-none mt-6 w-96 text-gray-200 normal-case tracking-wider font-normal hover:bg-gray-600" onClick={(e) => { UpdateBanner(e) }}>Update</button>
      </div>
    </div>
  );
};
