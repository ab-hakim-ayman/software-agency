...
const EditFeatureDesign = () => {
    ...
    useEffect(() => {
      axios.get(Base_URL+`/api/civil-admin/architecture/${id}/`, config)
        .then(res => {
    ...
    const UpdateFeatureDesign = (e) => {
      ...
      axios.patch(Base_URL+`/api/civil-admin/architecture/${id}/`, data1, config)
        .then(response => {
          // console.log(response.data)
          toast.success('FeatureDesign Edited Successfully')
          setTimeout(() => {
            navigate('/admin/civil/featuredesign')
          }, 1000);
          })
        .catch(err => console.log(err));
    }
    ...
    return (
      <div className=" bg-slate-950  px-10 w-full h-full mt-12 text-white">
        ...
        <div className="flex items-center justify-center mb-10">
          <button className="btn rounded h-8 bg-gray-500 shadow-inherit border-none mt-6 w-96 text-gray-200 normal-case tracking-wider font-normal hover:bg-gray-600" onClick={(e)=>{UpdateFeatureDesign(e)}}>
            Update
          </button>
        </div>
      </div>
...
export default EditFeatureDesign;