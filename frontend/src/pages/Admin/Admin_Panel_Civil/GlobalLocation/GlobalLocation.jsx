...
const GlobalLocation = () => {
  ...
  const DataFetch=()=>{
    axios.get(Base_URL+"/api/civil-admin/global-location/", config)
  ...
  const deleteButtonClick = (e, id) => {
    e.preventDefault()
    axios.delete(Base_URL+`/api/civil-admin/global-location/${id}/`, config)
      .then(res => {
  ...
  return (
    <div className=" bg-slate-950  px-10 w-full h-full mt-12 text-white">
      <div className="flex items-center justify-between">
        ...
        <Link to="/admin/civil/addGlobalLocation">
        ...
        <table className="table border-0 mt-10 h-[full] bg-slate-900 rounded-sm">
          ...
          <thead className="text-white  bg-slate-950">
            ...
            <th>SL</th>
              ...
              <th>
                <span className=" border-l-2  border-slate-200  pl-2">Office Location</span>
              </th>
              <th>
          ...
          {data?.data?.map((item, i) => {
              return <>
                ...
                <td class="px-6 py-4">
                  ...
                  <Link to={`/admin/civil/editGlobalLocation/${item?.id}/`}>
