...
const EditServices = () => {
  const [adminToken] = useState(localStorage.getItem('DTCAdminToken'));
    ...
    useEffect(() => {
      axios.get(Base_URL+`/api/civil-admin/services/${id}/`, config)
        .then(res => {
    ...
    const UpdateServices = (e) => {
      ...
      axios.patch(Base_URL+`/api/civil-admin/services/${id}/`, data1, config)
        .then(response => {
          // console.log(response.data)
          toast.success('Services Edited Successfully')
          setTimeout(() => {
            navigate('/admin/civil/services')
          }, 1000);
          })
        .catch(err => console.log(err));
    }
    ...
    return (
      <div className=" bg-slate-950  px-10 w-full h-full mt-12 text-white">
        ...
        <div>
          ...
          <p>Icon: <img src={data?.icon} e alt="" class="h-auto max-w-full rounded-lg w-60 h-70" /></p>
          <p>Path: { data?.path}</p>
