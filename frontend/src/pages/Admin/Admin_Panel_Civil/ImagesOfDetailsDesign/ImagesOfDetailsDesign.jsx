...
const ImagesOfDetailsDesign = () => {
  ...
  const DataFetch=()=>{
    // console.log("function calling");
    axios.get(Base_URL+"/api/civil-admin/images-details-design/", config)
  ...
  const deleteButtonClick = (e, id) => {
    e.preventDefault()
    axios.delete(Base_URL+`/api/civil-admin/images-details-design/${id}/`, config)
      .then(res => {
  ...
  return (
    <div className=" bg-slate-950  px-10 w-full h-full mt-12 text-white">
      <div className="flex items-center justify-between">
        ...
        Image Of Detail Design
        </h1>{" "}
        <Link to="/admin/civil/addImagesOfDetailsDesign">
        ...
        <table className="table border-0 mt-10 mb-10 h-[full] bg-slate-900 rounded-sm">
          ...
          {data?.data?.map((item, i) => {
              return <>
                ...
                <td class="px-6 py-4">
                   <img src={item?.img}e alt="" class="h-10 w-10" />
                </td>
                <td class="px-6 py-4">
                ...
                <td class="px-6 py-4">
                  {/* <Link to="#">
                    ...
                    <Link to={`/admin/civil/editImagesOfDetailsDesign/${item?.id}/`}>
...
export default ImagesOfDetailsDesign;
