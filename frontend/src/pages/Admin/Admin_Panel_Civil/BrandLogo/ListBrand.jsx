    ...
    const [userToken] = useState(localStorage.getItem('DTCAdminToken'));
      ...
      const handleDelete = async (e, id,activeValue) => {
        if(window.confirm('Are you sure ?')){
            e.preventDefault();
            await axios.patch(Base_URL+'/api/civil-admin/view-update-brand/' + id + '/', {active:activeValue?true:false}, {
              headers: {
      ...
      const handleUpdate = (e, id) => {
        navigate(`/admin/civil/update-brand-item/${id}/`);
      };
  
      const fetchData = async () => {
        try {
          await axios.get(Base_URL+'/api/civil-admin/create-view-brand/', {
            headers: {
              Authorization: 'Bearer ' + userToken
            }
          })
      ...
      return (
          ...
          <div className="flex items-center justify-between">
            ...
            <Link to="/admin/civil/add-brand-item">
            ...
            <table className="table border-0 mt-10 h-[full] bg-slate-900 rounded-sm">
              ...
              {itemsToShow.filter((item) =>{
                ...
                <tr key={t.id}>
                    ...
                    <td>{t.name}</td>
                    <td>{t.active ? "Active" : "Deactive"}</td>
                    <td>
                        <Button className="mr-2" variant="danger" onClick={event => handleDelete(event, t.id,t.active?false:true)}>
                          <RiDeleteBin5Line />
                        </Button>
                        <span>&nbsp;&nbsp;&nbsp;</span>
