  ...
  const [userToken] = useState(localStorage.getItem('DTCAdminToken'));
    ...
    useEffect(() => {
      const fetchData = async () => {
        try {
          await axios.get(Base_URL+`/api/civil-admin/view-update-brand/${id}/`, {
            headers: {
    ...
    const handleSubmit = (e) => {
      e.preventDefault();
      try {
        axios.patch(Base_URL+'/api/civil-admin/view-update-brand/'+ id + '/', formData, {
          headers: {
        ...
        .then((response) =>{
          if (response.data.type === 'success'){
            ...
            navigate('/admin/civil/brand-item')
          } else {
