...
const EditAccount = () => {
    ...
    useEffect(() => {
      axios.get(Base_URL+`/api/civil-admin/company-account/${id}/`, config)
        .then(res => {
          console.log("34 res.data", res.data);
          setData(res.data)
          setPaymentMethod(res.data.method_name_id)
          setPaymentMethod1(res.data.method_name)
          setBankName(res.data.bank_name)
          setCountry(res.data.country)
          setAccountDetail(res.data.account_details)
        ...
        .then(res => setDataP(res))
        .catch(err => console.log(err));
    },[])
  
    
    const UpdateCompanyAccount = (e) => {
      e.preventDefault()
      var data1 = {}
      if (bank_photo && bar_code_photo) {
        data1 = {
      ...
      axios.patch(Base_URL+`/api/civil-admin/company-account/${id}/`, data1, config)
        .then(response => {
          // console.log(response.data)
          toast.success('Company Account Edited Successfully')
          setTimeout(() => {
            navigate('/admin/civil/account')
          }, 1000);
          })
        .catch(err => console.log(err));
    }
    ...
    return (
      ...
      <label for="category" className="label my-2">
        ...
        <select id="categories" class="h-12 pl-4 w-full  bg-slate-950  border rounded border-neutral-500 " onChange={(e)=> setPaymentMethod(e.target.value)}>
          {datap?.data?.map((item, i) => { 
            return <>
              
              {
                paymentMethod1 === item?.method_name ?
        ...
        <input type="text" className="h-12 pl-4 w-full  bg-slate-950  border rounded border-neutral-500 " placeholder="Enter Bank Name"
        value={bankname}
        ...
        <textarea className="h-32 px-4 py-2 w-full  bg-slate-950  border rounded border-neutral-500 " placeholder="Enter Account Details"
        ...
        <div className="flex items-center justify-center mb-10">
          <button className="btn rounded h-8 bg-gray-500 shadow-inherit border-none mt-6 w-96 text-gray-200 normal-case tracking-wider font-normal hover:bg-gray-600" onClick={(e)=>{UpdateCompanyAccount(e)}}>
            Update
          </button>
        </div>
      </div>
