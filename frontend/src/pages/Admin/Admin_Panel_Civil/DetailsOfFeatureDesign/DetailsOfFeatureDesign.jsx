...
const DetailsOfFeatureDesign = () => {
  ...
  const deleteButtonClick = (e, id) => {
    console.log("50 call delete");
    e.preventDefault()
    axios.delete(Base_URL+`/api/civil-admin/details-feature-design/${id}/`, config)
      .then(res => {
  ...
  return (
    <div className=" bg-slate-950  px-10 w-full h-full mt-12 text-white">
      <div className="flex items-center justify-between">
        ...
        <Link to="/admin/civil/addDeatialofFeaturdesign">
  ...
  <span>Create</span>
      ...
      <div className="overflow-x-auto">
        <table className="table border-0 mt-10 mb-10 h-[full] bg-slate-900 rounded-sm">
          ...
          <tr className="border-0 grid grid-cols-7">
              <th>SL</th>
              <th>
                <span className=" border-l-2  border-slate-200  pl-2">ID</span>
              </th>
              <th>
                <span className=" border-l-2  border-slate-200  pl-2">Bed</span>
              </th>
              <th>
          ...
          {data?.data?.map((item, i) => {
              return <>
                <tr class="grid grid-cols-7 bg-slate border-b dark:bg-gray-800 dark:border-gray-700" key={i}>
                <td class="px-6 py-4">
                    {i+1}
                </td>
                <td class="px-6 py-4">
                ...
                <td class="px-6 py-4">
                  {/* <Link to="#">
                    ...
                    <Link to={`/admin/civil/editDeatialofFeaturdesign/${item?.id}/`}>
...
export default DetailsOfFeatureDesign;
