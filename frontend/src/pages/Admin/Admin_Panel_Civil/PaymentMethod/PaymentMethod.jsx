...
const PaymentMethod = () => {
  ...
  const deleteButtonClick = (e, id) => {
    e.preventDefault()
    axios.delete(Base_URL+`/api/civil-admin/payment-method/${id}/`, config)
      .then(res => {
  ...
  return (
    <div className=" bg-slate-950  px-10 w-full h-full mt-12 text-white">
      <div className="flex items-center justify-between">
        ...
        <Link to="/admin/civil/addPaymentMethod">
        ...
        <table className="table  border-[0.3px] border-slate-500  mt-10 h-[full]  bg-slate-900  rounded-sm">
          ...
          <tbody className="h-[50vh] overflow-scroll">
            {data?.data?.map((item, i) => {
              return <>
                <tr class="grid grid-cols-5 bg-slate border-b dark:bg-gray-800 dark:border-gray-700" keys={i}>
                <td class="px-6 py-4">
                    {i+1}
                </td>
                <td class="px-6 py-4">
                ...
                <td class="px-6 py-4">
                  ...
                  <td class="px-6 py-4">
                  <Link to={`/admin/civil/editPaymentMethod/${item?.id}/`}>
