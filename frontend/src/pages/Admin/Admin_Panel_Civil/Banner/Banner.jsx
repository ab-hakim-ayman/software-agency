...
const Banner = () => {
  ...
  const DataFetch=()=>{
    axios.get(Base_URL+"/api/civil-admin/banner-civil/", config)
  ...
  const deleteButtonClick = (e, id) => {
    e.preventDefault()
    axios.delete(Base_URL+`/api/civil-admin/banner-civil/${id}/`, config)
      .then(res => {
 ...
 <GrAdd className="bg-slate-100"></GrAdd>
  <span>Create</span>
      ...
      <div className="flex justify-between items-center mt-10 ml-1 uppercase text-lg tracking-wide">
        ...
        <div>
          <p onClick={DataFetch} className="flex items-center justify-center gap-2">
