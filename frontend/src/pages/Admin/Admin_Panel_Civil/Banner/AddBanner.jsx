...
const AddBanner = () => {
  ...
  const CreateBanner = (e) => {
    ...
    axios.post(Base_URL+"/api/civil-admin/banner-civil/", data, config)
      .then(response => {
  ...
  return (
    <div className=" bg-slate-950  px-10 w-full h-full mt-12 text-white">
      ...
      <div className="mt-8">
        ...
        <textarea className="h-12 pl-4 w-full  bg-slate-950  border rounded border-neutral-500 " placeholder="Enter Path"
        value={path}  accept="image/*"
        onChange={(e) => setPath(e.target.value)}
        > </textarea>

      </div>
      <div className="flex items-center justify-center mb-4">
        <button className="btn rounded h-8 bg-gray-500 shadow-inherit border-none mt-6 w-96 text-gray-200 normal-case tracking-wider font-normal hover:bg-gray-600" onClick={(e) => { CreateBanner(e) }}>Create</button>
      </div>
    </div>
  );
};
