...
const EditBanner = () => {
  ...
  useEffect(() => {
    axios.get(Base_URL+`/api/civil-admin/banner-civil/${id}/`, config)
      .then(res => {
  ...
  const UpdateBanner = (e) => {
    ...
    axios.patch(Base_URL+`/api/civil-admin/banner-civil/${id}/`, data1, config)
      .then(response => {
        console.log(response.data)
        toast.success('Banner edited Successfully')
        setTimeout(() => {
          navigate('/admin/civil/banner');
        }, 1000);
        })
      .catch(err => console.log(err));
  }
  ...
  return (
    <div className=" bg-slate-950  px-10 w-full h-full mt-12 text-white">
      ...
      <div className="mt-8">
        ...
        <textarea className="h-32 px-4 py-2 w-full  bg-slate-950  border rounded border-neutral-500 " placeholder="Enter Title"
          value={name}
