...
const PaymentPage = () => {
  ...
  useMemo(() => {
    const fetchData = async () => {
        try {
            await axios.get(Base_URL+'/api/it/account-view-all/', {
                headers: {
...
const handleSubmit = async(e) => {
  ...
  if (FormData.bank === '') {
    ...
    try{
      await axios.post(Base_URL+'/api/it/make-payment/', FormData,{
        headers: {
...
console.log(FormData)
  ...
  return (
    <div className="md:p-5 flex flex-col  justify-center items-center bg-white">
      ...
      <h1 className="w-full text-3xl text-center mt-4 mb-8 font-semibold"> Payment form for IT Products </h1>
      <div className="flex flex-col  justify-center">
        ...
        <Space className="my-2 flex">
          ...
          </button>
          {
            ...
            className=" h-full w-full max-w-xs p-3 bg-white border-[0.3px] border-blue-950 rounded-md focus:outline-none"
            />
          }
          {
            AllOrderData?.payment_left === 0 &&
