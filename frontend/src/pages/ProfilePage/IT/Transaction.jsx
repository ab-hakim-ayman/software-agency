...
const Order = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                await axios.get(Base_URL+'/api/it/get-all-transaction/', {
                    headers: {
    ...
    return (
        <div className=' h-screen'>
            <h1 className='text-xl text-center py-4 font-semibold'> IT Transaction List </h1>
                            ...
                            AllTransavtionData?.map((item, index) => {
                                return (
                                    <tr key={index} className={`${index%2 === 0 ? "bg-blue-50": "bg-gray-50"}`}>
                                        ...
                                        <td>{item?.amount}</td>
                                        <td>
                                            <div onClick={()=>{setImage(Base_URL+item?.pay_receipt_doc);setOpen(true);}} className='text-blue-950 font-semibold cursor-pointer px-3 py-2 border border-blue-950 hover:bg-blue-950 hover:text-white' >
                                                View
                                            </div>
                                            <Modal
                                                className='bg-white'
