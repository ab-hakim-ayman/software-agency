...
const ProfileDashboard = () => {
    ...
    useEffect(() => {
      const fetchData = async () => {
        try {
          await axios.get(Base_URL+'/api/civil/get-dashboard/', {
            headers: {
...
const Piedata = [
    ...
    return (
        <div className='py-8 h-full  bg-yellow-200 px-2'>
            <h3 className='text-slate-950 text-center font-bold text-3xl pt-2 pb-4'>Dashboard</h3>
            ...
            <div className='block w-full  py-8 lg:flex lg:gap-2'>
              <div className='rounded-md bg-yellow-400 text-slate-950 w-full pb-16 lg:w[50%]'>
                <h1 className='font-semibold text-center py-4'>Monthly Report</h1>
                  <BarChart width={700} height={400} data={data} margin={{top: 5,right: 30,left: 20,bottom: 10,}} >
                    ...
                    <ReferenceLine y={0} stroke="#0e0e0e" />
                    <Brush dataKey="name" height={30} stroke="#000" />
                    <Bar dataKey="pv" barSize={6} fill="#000" />
                    <Bar dataKey="uv" barSize={6} fill="#066dce" />
                  </BarChart>
              </div>
              <div className='mt-2 lg:mt-0 rounded-md bg-yellow-400 text-slate-950 w-full pb-16 lg:w[50%]'>
